...
class BacktestResult:
    trades: list[Trade]
    equity: np.ndarray                       # equity after each closed trade (per-trade)
    equity_index: list[pd.Timestamp]
    initial_balance: float
    final_balance: float
    # Mark-to-market equity at every bar close (includes unrealised P&L).
    # Use this for accurate drawdown and time-based Sharpe.
    bar_equity: np.ndarray = field(default_factory=lambda: np.array([], dtype=float))
    bar_equity_index: list[pd.Timestamp] = field(default_factory=list)
...
class BacktestEngine:
    def __init__(
        ...
        # Defensive: session_mask MUST line up positionally with df. A common
        # bug is to pass a full-data mask into a windowed df (see walkforward) —
        # the engine's .iloc lookup would then read the wrong bars silently.
        if session_mask is not None and len(session_mask) != len(df):
            raise ValueError(
                f"session_mask length ({len(session_mask)}) != df length ({len(df)}). "
                "Slice the mask to match the df window before passing it."
            )
        self.df_raw = df
    ...
    def run(self) -> BacktestResult:
        ...
        # bar-level mark-to-market equity (includes unrealised P&L of open positions)
        bar_eq: list[float] = [self.balance]
        bar_eq_idx: list[pd.Timestamp] = [df.index[0]]
        op = df["open"].to_numpy()
        hi = df["high"].to_numpy()
        lo = df["low"].to_numpy()
        ...
        # Initial sync — strategy.balance_ref so position sizing uses live balance.
        if hasattr(self.strategy, "_balance_ref"):
            self.strategy._balance_ref = self.balance
        ...
        for i in range(n - 1):
            ...
            if position is not None:
                ...
                if exit_price is not None:
                    ...
                    position = None
                    # FIX 5.1: compound — strategy sizing uses live balance
                    if hasattr(self.strategy, "_balance_ref"):
                        self.strategy._balance_ref = self.balance
            ...
            if signal is not None:
                ...
                elif action == "open" and position is None:
                    # session gate (no new entries when False)
                    if self.session_mask is None or bool(self.session_mask.iloc[i + 1]):
                        direction = int(signal["dir"])
                        spread = self._spread_price(i + 1)
                        # buy at ask = open + spread/2 + slip; sell at bid = open - spread/2 - slip
                        base = op[i + 1] + (spread / 2.0) * direction
                        entry = self._apply_slippage(base, direction)
                        position = Trade(
                            entry_idx=i + 1, exit_idx=-1,
                            entry_time=idx[i + 1], exit_time=idx[i + 1],
                            direction=direction,
                            entry_price=float(entry), exit_price=float("nan"),
                            sl=float(signal["sl"]), tp=float(signal["tp"]),
                            lots=float(signal["lots"]),
                            pnl=0.0, reason="open",
                        )

            # FIX 5.2: bar-end mark-to-market on EVERY bar (no early `continue`s
            # above any more) — captures unrealised drawdowns properly.
            if position is not None:
                unreal = self._pnl(position.direction, position.entry_price,
                                   float(cl[i + 1]), position.lots)
            else:
                unreal = 0.0
            bar_eq.append(self.balance + unreal)
            bar_eq_idx.append(idx[i + 1])
        ...
        return BacktestResult(
            ...
            bar_equity=np.array(bar_eq, dtype=float),
            bar_equity_index=bar_eq_idx,
